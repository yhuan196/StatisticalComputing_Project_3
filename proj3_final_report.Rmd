...
```
\begin{center}
Figure 1. the Count of Hurricanes in each Month
\end{center}
A bar plot has been used to visualize the number of hurricanes in each month. Based on the information provided in Figure 1, it appears that September has the highest number of hurricanes, while there are no hurricanes in February and March. 

```{r,echo=FALSE,out.width='90%',out.height='70%',fig.align='center'}
knitr::include_graphics("eda/fig4-1.png")
```
\begin{center}
Figure 2. the Average Speed of Hurriances in each Year
\end{center}
The hurricanes in the dataset have been compared based on their different nature ratings. This number of nature ratings is larger than the number of hurricanes due to the fact that some hurricanes may have different nature ratings at different times.

```{r,echo=FALSE,out.width='90%',out.height='70%',fig.align='center'}
knitr::include_graphics("eda/fig5-1.png")
```
\begin{center}
Figure 3. the Count of Hurricanes in each Nature
\end{center}
According to the information provided in Figure 3, it appears that over 50% of the nature ratings are classified as Tropical Storms. Additionally, this nature rating has the highest average wind speed, which is approximately 60 knots.
...
\end{center}
Figure 4 shows the Atlantic windstorm mean knot, so the windstorm mainly concentrates on the US. 
...
\end{center}
Figure 5 shows the Atlantic windstorm trajectories, so the windstorm mainly occurs in August and September. . 


## Data Pre-processing

- Filtered observations that occurred on a 6-hour intervals. (e.g., hour 0, 6, 12, 18)

- Created 3 new variables including the changes of latitude and longitude, as well as the wind speed between the time t and $t-6$ 
...
\scriptsize{
\begin{aligned}
&f( Y \mid \boldsymbol B,  \boldsymbol\mu, \sigma,  \boldsymbol\Sigma,\gamma) = \prod_{i = 1}^n f( Y_i |\boldsymbol B, \boldsymbol\mu, \boldsymbol\Sigma, \sigma,\gamma)=\\&\prod_{i=1}^n\frac{1}{\sqrt{2\pi}\sigma}\exp\{-\frac{1}{2}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)^\top (\sigma^2 I)^{-1}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)\}\\
...
\scriptsize{
\begin{aligned}
&f( \boldsymbol B \mid   \boldsymbol\mu,  \boldsymbol\Sigma) = \prod_{i = 1}^n f( \boldsymbol B |\boldsymbol\mu, \boldsymbol\Sigma)\propto\\&(|\boldsymbol\Sigma|)^{-\frac{N}{2}} \prod_{i=1}^n\exp\big\{-\frac{1}{2}((\beta_i-\mu)^\top \boldsymbol\Sigma^{-1}(\beta_i-\mu))\big\}
\end{aligned}
}
$$
...
&\propto \prod_{i=1}^n \exp\big\{-\frac{1}{2}[{\beta}_i-({Z}_i^\top(\sigma^2 I)^{-1}{Z}_i+{\boldsymbol A})^{-1}({Z}_i^\top(\sigma^2 I)^{-1}{Y}_i-{Z}_i^\top\gamma{X}_i(\sigma^2 I)^{-1} + {\mu}_i{\boldsymbol A})^\top]^\top \\
&\times ({Z}_i^\top(\sigma^2 I)^{-1}{Z}_i+{\boldsymbol A}) [{\beta}_i- ({Z}_i^\top(\sigma^2 I)^{-1}{Z}_i+{\boldsymbol A})^{-1}({Z}_i^\top(\sigma^2 I)^{-1}{Y}_i-{Z}_i^\top\gamma{X}_i(\sigma^2 I)^{-1} + {\mu}_i{\boldsymbol A})^\top]\big\}
\end{aligned}
$$
...
$\mu \sim MVN(M^{-1}N,M^{-1})$, where $M=NA- V^{-1}$ and $N=\sum_i^N(\boldsymbol A\beta_i)$

3. The posterior distribution of ${\Sigma}$

$$
\begin{aligned}
\pi({\boldsymbol\Sigma}|{\boldsymbol B} ,{\boldsymbol\mu},{\gamma},\sigma,{Y}) \propto  {|\boldsymbol\Sigma|}^{-\frac{(N+v+d+1)}{2}}\exp\{-\frac{1}{2}(\sum_i^N({\beta_i-\mu})^\top\boldsymbol \Sigma^{-1} ({\beta_i-\mu})+tr({\boldsymbol S\boldsymbol \Sigma^{-1}}))\}\\
...
\end{aligned}
$$
$\gamma \sim MVN(M^{-1}N, M^{-1})$, where $M =\frac{\sum_i^NX_i^\top X_i}{\sigma^2}+400I$ and $N=\frac{\sum_i^N(X_i^\top Y_i-X_i^\top Z_i\beta_i^{T})}{\sigma^2}$ 
...
## Analysis of the Fix Effect $\boldsymbol\gamma$
### MCMC convergence
