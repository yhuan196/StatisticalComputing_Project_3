...
Let $B = ({\beta}_1^\top,..., {\beta}_n^\top)^\top$, derive the posterior distribution of the parameters $\Theta = ({B}^\top, {\mu}^\top, \sigma^2, \Sigma, \gamma)$.
\
\
Let ${Z}_i(t){\beta}_i^\top = \beta_{0,i} + \beta_{1,i}Y_i(t) + \beta_{2,i}\Delta_{i,1}(t) + \beta_{3,i}\Delta_{i,2}(t) + \beta_{4,i}\Delta_{i,3}(t) + \epsilon_{i}(t)$
...
\scriptsize{
\begin{aligned}
&f( Y \mid  B,  \mu, \sigma,  \Sigma,\gamma) = \prod_{i = 1}^n f( Y_i | B, \mu, \Sigma, \sigma)=\\&\prod_{i=1}^n\frac{1}{\sqrt{2\pi}\sigma}\exp\{-\frac{1}{2}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)^\top (\sigma^2 I)^{-1}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)\}\\
...
\scriptsize{
\begin{aligned}
&f( B \mid   \mu,  \Sigma) = \prod_{i = 1}^n f( B |\mu, \Sigma)=\\&\prod_{i=1}^n\frac{1}{\sqrt{2\pi|\Sigma|}}\exp\{-\frac{1}{2}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)^\top (\sigma^2 I)^{-1}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)\}\\
&\times  (|\Sigma|)^{-\frac{N}{2}} \prod_{i=1}^n\exp\big\{-\frac{1}{2}((\beta_i-\mu)^\top \Sigma^{-1}(\beta_i-\mu))\big\}
\end{aligned}
}
$$
...
&\propto \prod_{i=1}^n \exp\big\{-\frac{1}{2}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma_i)^\top (\sigma^2 I)^{-1}({Y}_i - {Z}_i{\beta}_i^{T}-{X_i}\gamma)\big\}+(\beta_i-\mu)^\top A(\beta_i-\mu)\}\\
...
$$
\begin{aligned}
\pi(\gamma| B,\mu, \Sigma,\sigma,{Y})&\propto \prod_{i=1}^N\exp\{-\frac{1}{2}(Y_i-Z_i\beta_i^{T}-X_i\gamma)^\top(\sigma^2I)^{-1}(Y_i-Z_i\beta_i^{T}-X_i\gamma)\} \times \exp\{-\frac{400\gamma^\top\gamma}{2}\}\\
...
### Fix Effect Parameter $\boldsymbol\gamma$
