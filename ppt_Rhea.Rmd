...
$$
where $N=\frac{Z_i^\top Z_i}{\sigma^2}+\Sigma^{-1}$ and $M=\frac{Y_i^\top Z_i^\top-X_i^\top Z_i\gamma}{\sigma^2}+\mu^\top \Sigma^{-1}$
$$
\scriptsize{
\begin{aligned}
...
$$
where $N=NA+\frac{1}{v}$ and $M=\sum_i^n\beta_iA$, and v is the degree of freedom.
...
\scriptsize{
\begin{aligned}
\gamma \sim MVN(M^{-1}N,M^{-1})
\end{aligned}
}
$$
where $N=\sum_i^nX_i^\top\sigma^{-1}IX_i+400I$ and $M=\frac{\sum_i^nX_i^\top Y_i-X_i^\top Z_i\beta_i}{\sigma^2}$
