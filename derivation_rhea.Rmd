...
&\propto \prod_{i=1}^n \exp\big\{-\frac{1}{2}[\boldsymbol{\beta}_i-(A+\boldsymbol{Z}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i)^{-1}(\boldsymbol{Y}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i+\gamma\boldsymbol{X}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i + \boldsymbol{\mu}_i^\top\boldsymbol{A})^\top]^\top (A+\boldsymbol{Z}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i)\\
& [\boldsymbol{\beta}_i- (A+\boldsymbol{Z}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i)^{-1}(\boldsymbol{Y}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i+\gamma\boldsymbol{X}_i(\sigma^2 I)^{-1}\boldsymbol{Z}_i + \boldsymbol{\mu}_i^\top\boldsymbol{A})^\top]\big\}
\end{aligned}
$$
...
$$
\begin{aligned}
f(\mu_t|B_t, \sigma^2, A, \gamma) & \propto P(B\mid \boldsymbol{\mu}, \boldsymbol{\Sigma})P(\boldsymbol\mu)\\
\propto \exp\{-\frac{1}{2}\sum_i^n(\beta_j-\mu)^\top A(\beta_j-\mu)\}*\exp\{\frac{\mu^2}{-2v}\}\\
\propto \exp\{-\frac{1}{2}(\mu^\top(NA)\mu-2(\sum_i^n\beta_j A)\mu)\}\\
\propto \exp\{-\frac{1}{2}(\mu-(NA-\frac{1}{v})^{-1}(\sum_i^n\beta_jA)^\top)(NA-\frac{1}{v})*(\beta-(NA-\frac{1}{v})^{-1}(\sum_i^n\beta_jA)^\top)\}
\end{aligned}
$$

$MVN_d((NA+\frac{1}{v})^{-1}(\sum_i^n\beta_jA)^\top,(NA+\frac{1}{v})^{-1})$


3. Generate $\sigma_t^2$ from $f(\sigma_t|B_t, \mu_t, A_{t-1}, \gamma_{t-1})$

$$
f(\sigma|B, \mu, \Sigma, \gamma,Y) \propto \prod_{i=1}^n\exp\{-\frac{1}{2}(Y_i-Z_i\beta_i-X_i\gamma)^\top(\sigma^2I)^{-1}(Y_i-Z_i\beta_i-X_i\gamma)\} \times \exp\{-\frac{400\gamma^\top\gamma}{2}\}\\
...
Thus, we have $$f(\sigma|B, \mu, \Sigma, \gamma,Y) \sim ((X_i^\top\sigma^{-2}IX_i+400I)^{-1}(X_i^\top\sigma^{-2}IY_i-X_i^\top\sigma^{-2}IZ_i\beta_i),(X_i^\top\sigma^{-2}IX_i+400I)^{-1})$$

4. Generate $A_t$ from $f(A_t|B_t, \mu_t, \sigma_t^2, \gamma_{t-1})$
$$
\pi(\boldsymbol{\Sigma}|\boldsymbol{B^\top} ,\boldsymbol{\mu^\top},\boldsymbol{\gamma^\top},\sigma,\boldsymbol{Y^\top}) \propto \boldsymbol{|\Sigma|}^{-\frac{(n+v+6)}{2}}\exp\{-\frac{1}{2}(\sum_i^n(\boldsymbol{\beta_i-\mu})+tr(\boldsymbol{SE})^{-1})\}\\\propto \boldsymbol{|\Sigma|}^{-\frac{(n+v+6)}{2}} \exp\{-\frac{1}{2}tr( \boldsymbol{S+\sum_i^n(\beta_i-\mu)(\beta_i-\mu)^\top}\Sigma^{-1})\}
$$
Thus, we have
$$\boldsymbol{\Sigma|(B^\top,\mu^\top,\gamma^\top},\sigma,\boldsymbol{Y^\top}) \sim w^{-1}(\boldsymbol{S+\sum_i^n(\beta_i-\mu)(\beta_i-\mu)^\top,n+v})  $$

5. Generate $\gamma$ from $f(\gamma|B_t, \mu_t, \sigma_t^2, A_t)$
$$
f(\gamma|B,\mu, \Sigma,\sigma^2)\propto \exp\{\frac{\sum_i^n-(Y_j-Z_j\beta_j-X_j\gamma)^\top (Y_j-Z_j\beta_j-X_j\gamma)}{2\sigma^2}-200\sigma^2\}\\
...
$$
MVN((\frac{X_j^\top X_j}{\sigma^2}+400I)^{-1}(\sum_i^nY_j^\top X_j-\sum_i^nX_jZ_j\beta_j)^\top,(\frac{X_j^\top X_j}{\sigma^2}+400I)^{-1})
$$
