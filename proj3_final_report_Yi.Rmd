...
We have tested different start values for MCMC algorithm, the result chain behave similarly. We finally decide to initialize the parameters by using the results from fitting generalized linear mixed model in R. $\boldsymbol B$ is a $5 \times 699$ matrix, $\boldsymbol \mu$ is a $5 \times 1$ matrix, $\sigma$ is a number, $\boldsymbol \Sigma$ is a $5 \times 5$ matrix, and $\boldsymbol \gamma$ is a $14 \times 1$ matrix.
...
```
\begin{center}
Figure 2 The ACF, Convergence, Histogram of $\beta_{HurricaneGeorge1951}$
\end{center}
