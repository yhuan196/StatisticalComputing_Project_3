...
gamma=function(B,zdat, ydat,xdat, sigma) {
  ...
  for (i in 1:n){
    ...
    y = as.matrix(ydat[[i]])
    x=  as.matrix(xdat[[i]])
  a = sigma^(-2) * (t(x) %*% x) + 400*diag(15)
  b = solve(a)
  c = sigma^(-2)*as.matrix((colSums(t(y)%*%x))-as.matrix(colSums(t(x)%*%z%*%as.matrix(B[,i])))) 
