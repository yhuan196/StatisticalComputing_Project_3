...
# EDA-Count of Hurricanes in each Month/Year/Nature
...
bx=storms_month_name %>% 
  ...
  ggplot(aes(x = Month)) +
  geom_bar()
bxp=bx+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

storms_season_name = distinct(group_by(dt_long, Season, ID), Season)
dp=ggplot(data = storms_season_name) + 
  geom_bar(aes(x = Season)) +
  scale_x_continuous("Year")

storms_nature_name = distinct(group_by(dt_long, Nature, ID), Nature)
lp=ggplot(data = storms_nature_name) + 
...
# the hurrican data in the world map
```{r}
library(maps)
...
# MCMC Algorithm
1. $f(\textbf{B} |\boldsymbol{\mu^\top}, \sigma^2, A, \gamma, Y^\top) \sim MVN_d((\frac{Z_j^\top Z_j}{\sigma^2}+A)^{-1}(\frac{Y_j^\top Z_j^\top-X_j^\top Z_j\gamma}{\sigma^2}+\mu^\top A)^\top,\frac{Z_j^\top Z_j}{\sigma^2}+A)^{-1})$
2. $f(\mu_t|B^\top, \sigma^2, A, \gamma, Y^\top) \sim MVN_d((NA+\frac{1}{v})^{-1}(\sum_i^n\beta_jA)^\top,(NA+\frac{1}{v})^{-1})$
3. $f(\sigma|B^\top, \mu^\top, A, \gamma, Y^\top)\sim MVN_d((X_i^\top\sigma^{-2}IX_i+400I)^{-1}(X_i^\top\sigma^{-2}IY_i-X_i^\top\sigma^{-2}IZ_i\beta_i),(X_i^\top\sigma^{-2}IX_i+400I)^{-1})$
4. $f(\boldsymbol{A}|\boldsymbol{B^\top} \sigma^2, \boldsymbol{\gamma, Y^\top}) \sim w^{-1}(\boldsymbol{S+\sum_i^n(\beta_i-\mu)(\beta_i-\mu)^\top,n+v})$
5. $f(\gamma|B^\top,\mu^top, \sigma^2, A, Y^\top) \sim MVN((\frac{X_j^\top X_j}{\sigma^2}+400I)^{-1}(\sum_i^nY_j^\top X_j-\sum_i^nX_jZ_j\beta_j)^\top,(\frac{X_j^\top X_j}{\sigma^2}+400I)^{-1})$
